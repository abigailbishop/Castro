...
module extern_probin_module
  ...
  attributes(managed) :: eos_ttol
  attributes(managed) :: eos_dtol
  attributes(managed) :: small_x
  attributes(managed) :: use_tables
  attributes(managed) :: use_c12ag_deboer17
  attributes(managed) :: ode_max_steps
  attributes(managed) :: scaling_method
  attributes(managed) :: use_timestep_estimator
  attributes(managed) :: ode_scale_floor
  attributes(managed) :: ode_method
  attributes(managed) :: safety_factor
  attributes(managed) :: do_constant_volume_burn
  attributes(managed) :: call_eos_in_rhs
  attributes(managed) :: dT_crit
  attributes(managed) :: burning_mode
  attributes(managed) :: burning_mode_factor
  attributes(managed) :: integrate_temperature
  attributes(managed) :: integrate_energy
  attributes(managed) :: jacobian
  attributes(managed) :: centered_diff_jac
  attributes(managed) :: burner_verbose
  attributes(managed) :: rtol_spec
  attributes(managed) :: rtol_temp
  attributes(managed) :: rtol_enuc
  attributes(managed) :: atol_spec
  attributes(managed) :: atol_temp
  attributes(managed) :: atol_enuc
  attributes(managed) :: retry_burn
  attributes(managed) :: retry_burn_factor
  attributes(managed) :: retry_burn_max_change
  attributes(managed) :: abort_on_failure
  attributes(managed) :: renormalize_abundances
  attributes(managed) :: SMALL_X_SAFE
  attributes(managed) :: MAX_TEMP
  attributes(managed) :: react_boost
  attributes(managed) :: reactions_density_scale
  attributes(managed) :: reactions_temperature_scale
  attributes(managed) :: reactions_energy_scale
#endif
